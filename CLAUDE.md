...
- Three workflows over one items table (Miniflux + Linkding + Readeck): Feeds (`source=feed`), Read Later (`source=manual`, `read_later=1`, full article extracted), and Bookmarks (`source=manual`, `read_later=0`, link-only). Adding a link (`/save`) has a "Read later" checkbox: ticked runs the Saver (fetch + extract + snapshot), unticked runs `Saver.SaveLink` (link + title only). A bookmark can be promoted in the reader (`/items/{id}/readlater`), which extracts in place. Do not conflate bookmark and read-later; that distinction is the point.
- Views are `?view=` tabs (`viewOrder`/`viewsByKey`): feeds, later, bookmarks, starred, archived, all; store.ListPage filters by Source/State/ReadLater/Shared. Bare `/` renders the dashboard (`dashboard`), not a list. Star/archive/share are reversible toggles posting an explicit `!= "0"` value; the star gesture is Star/Starred (a favourite flag), Share publishes an item, Archive files it out of its list but keeps it searchable and returns via a `safeReturn`-checked `return` field.
