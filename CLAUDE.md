...
- Feed polling: internal/feeds Service; main starts `Run(ctx, time.Minute)` as a goroutine, which polls due feeds with conditional GET (ETag/Last-Modified) and disables a feed after 5 consecutive failures. A feed URL that permanently redirects (301/308, detected via `fetch.Client.GetTrackingRedirects`) is rewritten in place instead of left to erode the failure counter.
