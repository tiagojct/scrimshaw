...
- Management: `/feeds` lists subscriptions (unsubscribe via `POST /feeds/{id}/delete`, which keeps items — feed_id is set null). The reader edits tags (`POST /items/{id}/tags`, replace semantics via store.SetItemTags) and deletes an item (`POST /items/{id}/delete`, which cascades highlights/tags/FTS and removes the snapshot file).
