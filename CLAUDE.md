...
- Item states and workflow views: the datastore is unified (one items table) but the two workflows are separated in the UI, not the schema. The index has tabs driven by `?view=` (see `viewOrder`/`viewsByKey` in internal/server): Feeds (`source=feed`, the default home), Read Later (`source=manual`, saved pages and imported bookmarks), Starred, Archived, and All. store.ListPage filters by Source and/or State. "Save a page" (`/save`) ingests a URL into Read Later; the star gesture (labelled Star/Starred) is a favourite flag across both sources; Archive files a read item out of its list but keeps it searchable. Star and archive are reversible toggles: the reader posts an explicit `starred`/`archived` value (`!= "0"`). Archive returns to the item's own list via a `return` field, passed through `safeReturn` (rejects non-same-origin paths).
