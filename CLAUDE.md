...
- Reading files an item away: `SetReadState('read')` also stamps `read_at` and archives; `'unread'` clears both and un-archives. So marking read removes an item from its active list and it shows up in Archived (still searchable). The reader's primary button is "Mark read" (returns to the item's list) or, when archived, "Move to inbox".
