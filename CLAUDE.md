...
- Item states and views: an item is unread/read, starred, and archived (SPEC states). The index exposes them as views via `?state=` (Inbox = not archived, Unread, Saved = starred, Archived = archived); store.ListPage branches on State. "Save" is the UI label for the star gesture (keep it), distinct from `/save` which ingests a new URL; "Archive" files a read item out of the inbox but keeps it searchable. Star and archive are reversible toggles: the reader posts an explicit `starred`/`archived` value (`!= "0"`), so the same handler stars/unstars and archives/unarchives.
